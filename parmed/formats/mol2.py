...
from parmed.constants import (PARMED_ELECTROSTATIC, AMBER_ELECTROSTATIC,
        CHARMM_ELECTROSTATIC, GROMACS_ELECTROSTATIC, OPENMM_ELECTROSTATIC)
...
class Mol2File(object):
    ...
    @staticmethod
    def parse(filename, structure=False, program=None):
        ...
        program : str, optional
            This can be either 'amber', 'charmm', 'gromacs', or 'openmm'
            (case-insensitive). This keyword defines *which* program's
            electrostatic force constant will be used to scale the defined
            charges. If this is a mol2 file generated by antechamber for use in
            gaff, for instance, then this should be set to 'amber'. By default,
            no scaling is done (when program is None).
            ...
            for line in f:
                ...
                if section == 'ATOM':
                    ...
                    if 'NO_CHARGES' not in mol_info:
                        try:
                            charge = float(words[8]) * chg_scale
                        except IndexError:
                            charge = 0
                    else:
                        charge = 0
    ...
    @staticmethod
    def write(struct, dest, mol3=False, split=False, program=None):
        ...
        if split:
            ...
            if isinstance(struct, ResidueTemplateContainer):
                for res in struct:
                    Mol2File.write(res, dest, mol3, program=program)
                return
