...
class AmoebaParm(AmberParm):
    ...
    def mdin_skeleton(self):
        ...
        return ('Input file for AMOEBA simulations.\n'
                ...
                ' /\n' % (bool(self.bond_list), bool(self.urey_bradley_list),
                          bool(self.angle_list), bool(
                              self.trigonal_angle_list),
                          bool(self.oopbend_list), bool(self.dihedral_list),
                          bool(self.pitorsion_list), bool(
                              self.stretch_bend_list),
    ...
    def _load_urey_bradley_info(self):
        ...
        if not 'AMOEBA_UREY_BRADLEY_LIST' in self.parm_data:
            return
        data = self.parm_data
        ...
        for i, j, k, l, m, n, o in zip(it, it, it, it, it, it, it):
            self.pi_torsions.append(
                PiTorsion(self.atoms[i - 1], self.atoms[j - 1], self.atoms[k - 1],
                          self.atoms[
                              l - 1], self.atoms[m - 1], self.atoms[n - 1],
    ...
    def _load_stretch_bend_info(self):
        ...
        if 'AMOEBA_STRETCH_BEND_FORCE_CONSTANT' in data:
            for a, b, c, d in zip(data['AMOEBA_STRETCH_BEND_FORCE_CONSTANT'],
                                  data[
                                      'AMOEBA_STRETCH_BEND_BOND1_EQUIL_VALUE'],
                                  data[
                                      'AMOEBA_STRETCH_BEND_BOND2_EQUIL_VALUE'],
    ...
    def _xfer_urey_bradley_info(self):
        ...
        data['AMOEBA_UREY_BRADLEY_FTAB_DEGREE'] = [
    ...
    def _xfer_frame_info(self):
        ...
        if len(self.chiral_frames) > 0:
            ...
            for cf in self.chiral_frames:
                clist.extend(
                    [cf.atom1.idx + 1, cf.atom2.idx + 1, cf.chirality])
        else:
    ...
    def _xfer_exception_info(self):
        ...
        for adj in self.adjusts:
            alist.extend(
                [adj.atom1.idx + 1, adj.atom2.idx + 1, adj.type.idx + 1])
