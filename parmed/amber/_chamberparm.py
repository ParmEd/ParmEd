...
class ChamberParm(AmberParm):
    ...
    def load_pointers(self):
        ...
        self.pointers['NIMPRTYPES'] = self.parm_data[
            'CHARMM_NUM_IMPR_TYPES'][0]
        # If CMAP is not present, don't load the pointers
        if self.has_cmap:
            self.pointers['CMAP'] = self.parm_data['CHARMM_CMAP_COUNT'][0]
            self.pointers['CMAP_TYPES'] = self.parm_data[
                'CHARMM_CMAP_COUNT'][1]
    ...
    def fill_LJ(self):
        ...
        # we need to raise some numbers to the 1/6th power
        one_sixth = 1.0 / 6.0

        for i in range(ntypes):
            lj_index = self.parm_data[
                "NONBONDED_PARM_INDEX"][ntypes * i + i] - 1
    ...
    def recalculate_LJ(self):
        ...
        for i in range(ntypes):
            for j in range(i, ntypes):
                index = self.parm_data[
                    'NONBONDED_PARM_INDEX'][ntypes * i + j] - 1
