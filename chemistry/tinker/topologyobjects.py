...
class Atom(object):
   ...
   def __repr__(self):
      return '<Atom %s [#%d]; Mass=%.2f; "%s">' % (self.symbol,
            self.atomic_number, self.mass, self.desc)
   ...
   def __str__(self):
      retstr =  ('Atom %s [#%s] %s\n'
                 ...
                 '   Class:       %d\n'
                 '   Mass:        %.2f\n'
                 '   Valence:     %d' % (self.symbol, self.atomic_number,
                    self.desc, self.type, self.class_, self.mass, self.valence)
      )
...
class AngleBend(object):
   ...
   def __repr__(self):
      retstr = '<AngleBend [%s-%s-%s]; k=%.2f; Theta_eq=%.2f' % (
            self.atom1.symbol, self.atom2.symbol, self.atom3.symbol, self.k,
            self.theteq
      )
   ...
   def __repr__(self):
      return ('<StretchBend [%s-%s-%s]; k=%.2f; Theta_eq=%.2f; R1eq=%.2f; '
            'R2eq=%.2f>' % (self.atom1.symbol, self.atom2.symbol,
               self.atom3.symbol, self.k, self.theteq, self.r1eq, self.r2eq))
...
class OutOfPlaneBend(object):
   ...
   def __repr__(self):
      return '<%s [%s-%s-%s-%s]; k=%.2f>' % (type(self).__name__,
            self.atom1.symbol, self.atom2.symbol, self.atom3.symbol,
            self.atom4.symbol, self.k)
...
class TorsionAngle(object):
   ...
   def __init__(self, atom1, atom2, atom3, atom4, args):
      ...
      if len(args) % 3 != 0:
         raise TypeError('TorsionAngle expects an equal number of Amplitudes, '
                         'phases, and periodicities.')
      nterms = len(args) // 3
      ...
      if (len(self.amplitude) != len(self.phase) or
         ...
         raise RuntimeError('BUG -- inconsistent number of terms in torsion')

   def __repr__(self):
      if len(self.amplitude) == 0:
         return '<TorsionAngle [%s-%s-%s-%s]; No Terms>' % (self.atom1.symbol,
               self.atom2.symbol, self.atom3.symbol, self.atom4.symbol)
      return '<TorsionAngle [%s-%s-%s-%s]; Ampl=%s; Phase=%s; Per=%s>' % (
            self.atom1.symbol, self.atom2.symbol, self.atom3.symbol,
            self.atom4.symbol, self.amplitude, self.phase, self.periodicity)
...
class PiTorsion(object):
   ...
   def __str__(self):
      return ('PiTorsion %r --- %r\n'
              '   Amplitude = %.4f' % (self.atom1, self.atom2, self.amplitude))
...
class TorsionTorsion(object):
   ...
   def __repr__(self):
      return '<TorsionTorsion [%s-%s-%s-%s-%s]; Spline=(%d,%d)>' % (
            self.atom1.symbol, self.atom2.symbol, self.atom3.symbol,
            self.atom4.symbol, self.atom5.symbol, self.spline1, self.spline2)

   def __str__(self):
      return ('TorsionTorsion %r --- %r --- %r ---\n'
              '                      %r --- %r\n'
              '   Spline Grid = (%d x %d)' % (self.atom1, self.atom2,
                 self.atom3, self.atom4, self.atom5, self.spline1,
                 self.spline2))
...
class AtomicMultipole(object):
   ...
   def __repr__(self):
      return '<AtomicMultipole [%s] %s; Frame=%s; Moments=%s>' % (
               self.atom.symbol, self.definition, self.frame, self.moment)
   ...
   def __str__(self):
      return ('AtomicMultipole %r "%s"\n'
              ...
              '             %8.5f\n'
              '             %8.5f %8.5f\n'
              '             %8.5f %8.5f %8.5f' % (self.atom, self.definition,
                 self.frame, self.moment[0], self.moment[1], self.moment[2],
                 self.moment[3], self.moment[4], self.moment[5], self.moment[6],
                 self.moment[7], self.moment[8], self.moment[9]))
