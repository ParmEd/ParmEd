...
class KeywordControlFile(object):
   ...
   def __init__(self, fname):
      ...
      self._datatypes = {'PARAMETERS' : str, 'A-AXIS' : float, 'B-AXIS' : float,
            'C-AXIS' : float, 'ALPHA' : float, 'BETA' : float, 'GAMMA' : float}
      # Parse the file
      for line in open(fname, 'r'):
         ...
         try:
            ...
            raise TinkerKeyFileError('Malformed keyword control file! '
                     'Could not convert the value of %s into type %s' % (key,
                        self._datatypes[key]))
...
class XyzFile(object):
   ...
   class _Atom(object):
      ...
      def __init__(self, name, x, y, z, type, bonded_partners):
         ...
         self.type = int(type)
         self.bonded_partners = [int(i) for i in bonded_partners] # index from 1
   ...
   def read(self, fname):
      ...
      with open(fname, 'r') as f:
         if f.readline().strip() != 'Number of Atoms and Title :':
            raise TinkerDynFileError('%s is not a recognized TINKER .dyn file'
                     % fname)
         line = f.readline()
         ...
         if line == 'Current Translational Velocities :':
            ...
            if f.readline().strip() != 'Current Angular Velocities :':
               raise TinkerDynFileError('Could not find Angular velocity '
                        'section in .dyn file')
            self.angular_velocities = [[0.0, 0.0, 0.0] for i in
                        xrange(self.natom)]
            ...
            if f.readline().strip() != 'Current Angular Momenta :':
               raise TinkerDynFileError('Could not find angular momenta '
                        'section in .dyn file')
            self.angular_momenta = [[0.0, 0.0, 0.0] for i in xrange(self.natom)]
            ...
            if f.readline().strip() != 'Current Atomic Accelerations :':
               raise TinkerDynFileError('Could not find accelerations in %s ' %
                        fname)
            self.accelerations = [[0.0, 0.0, 0.0] for i in xrange(self.natom)]
            DynFile._read_section(f, self.accelerations, self.natom)
            if f.readline().strip() != 'Alternate Atomic Accelerations :':
               raise TinkerDynFileError('Could not find old accelerations in %s'
                        % fname)
            self.old_accelerations = [[0.0, 0.0, 0.0] for i in
                        xrange(self.natom)]
