...
class TinkerAnalout(object):
   ...
   def read(self, fname):
      ...
      with open(self.fname, 'r') as f:
         ...
         while True:
            if not line:
               raise TinkerAnaloutError('Could not find the TINKER watermark '
                     'in %s' % fname)
         ...
         while not line.lstrip().startswith('Total Numbers of Atoms and Interactions'):
            if not line:
               raise TinkerAnaloutError('Could not find the atom/interaction count')
            line = f.readline()
         # Eat the next line
         f.readline()
         line = f.readline()
         ...
         while line.strip():
            try:
               ...
               raise TinkerAnaloutError('Unrecognized pointer keyword %s' % key)
            try:
               self.pointers[key] = int(line[27:].strip())
            except ValueError:
               raise TinkerAnaloutError('Could not convert pointer %s to int'
                        ' [%s]' % (key, line.rstrip()))
            except KeyError:
               raise Exception('Should not be here -- internal flag key error')
            line = f.readline()
         ...
         while line.strip() != 'Atom Type Definition Parameters :':
            if not line:
               raise TinkerAnaloutError('Unexpected EOF when looking for atom '
                        'definitions')
            line = f.readline()
         TinkerAnalout._read_atom_definitions(self, f)
         # Get all of the sections defined in _functionmap -- see bottom of this
         # file
         while True:
            line = f.readline()
            try:
...
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
   ...
   def _read_vdw(self, f):
      ...
      for i in range(self.pointers['natom']):
         ...
         line = f.readline()
         if atnum != i + 1:
            raise TinkerAnaloutError('Atom number mismatch in vdW [%d vs %d]'
                  % (i + 1, atnum))
   ...
   def _read_strbnd(self, f):
      ...
      for i in range(self.pointers['nstrbnd']):
         try:
            ...
            at2 = int(line[15:21]) - 1
            at3 = int(line[21:27]) - 1
            self.stretchbend_list.add(self.atom_list[at1], self.atom_list[at2],
                  self.atom_list[at3], line[27:40], line[40:50], line[50:60],
                  line[60:70])
         except ValueError:
   ...
   def _read_ureybrad(self, f):
      ...
      for i in range(self.pointers['nurey']):
         try:
            ...
            at1 = int(line[9:15]) - 1
            at2 = int(line[15:21]) - 1
            # Support older-style analyze output that had all 3 atoms involved
            # in the angle. Newer analyze output contains only the first and 3rd
            # atoms, so that is all we pass into the UreyBradleyList function
            try:
               at3 = int(line[21:27]) - 1
            except ValueError:
               at3 = at2
      ...
      for i in range(self.pointers['nopbend']):
         try:
            ...
            self.oopbend_list.add(self.atom_list[at1], self.atom_list[at2],
                  self.atom_list[at3], self.atom_list[at4], line[42:52])
         except ValueError:
            raise TinkerAnaloutError('Error parsing out-of-plane bending term')
         line = f.readline()
   ...
   def _read_opdist(self, f):
      ...
      for i in range(self.pointers['nopdist']):
         try:
            ...
            raise TinkerAnaloutError('Error parsing out-of-plane distance term')
         line = f.readline()
      ...
      for i in range(self.pointers['ntors']):
         try:
            ...
            self.torangle_list.add(self.atom_list[at1], self.atom_list[at2],
                  self.atom_list[at3], self.atom_list[at4], terms)
         except ValueError:
   ...
   def _read_dipoles(self, f):
      ...
      for i in range(self.pointers['npole']):
         try:
            ...
            raise TinkerAnaloutError('Error parsing dipole polarizabilities')
         line = f.readline()
   ...
   def _read_interactions(self, f):
      ...
      while line.strip():
         try:
            ...
            raise TinkerAnaloutError('Unrecognized token in interaction count '
                  '[%s]' % line[1:20].strip())
