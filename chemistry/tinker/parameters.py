...
class _FourierAngleType(_AngleType):
   ...
   def __repr__(self):
      return '<_FourierAngleType: k=%s; theteq=%s; periodicity=%s>' % (self.k,
                     self.theteq, self.periodicity)
...
def get_atom_type(index, atomic_number, mass, valence):
   """
   Factory for getting an _AtomType, but making sure that only one instance of a
...
class AmoebaParameterSet(object):
   ...
   """
   atomre = re.compile(r'atom *(\d+) *(\d+) *([A-Za-z\-\+]+) *"(.+)" *'
                       r'(\d+) *(\d+\.\d+) *(\d+)', re.I)
