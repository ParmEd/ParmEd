...
class _AmberAsciiCoordinateFile(object):
   ...
   def __init__(self, fname, natom, hasbox, mode='r', title=None):
      ...
      Notes:
         ...
         bzip2, and compression is determined automatically by filename
      ...
      if mode == 'r':
         ...
         # coordinates or the box for this particular frame. Each frame must be
         # written as coordinates first, then box.
...
class AmberAsciiRestart(_AmberAsciiCoordinateFile):
   ...
   def _parse(self):
      ...
      This method is called automatically for 'old' restart files and should not
      be called by external callers
      """
      ...
      while not lines[-1].strip():
         lines.pop()
      # Determine what information we have based on the number of lines present
      if len(lines) == int(ceil(self.natom / 2.0) + 2):
         ...
         raise RuntimeError('Badly formatted restart file. Has %d lines for %d '
                            'atoms.' % (len(self.lines), self.natom))
      if np is not None:
      ...
      if self.hasbox:
         ...
         self._cell_lengths[0:3] = converter([float(line[0:12]),
            float(line[12:24]), float(line[24:36])])
         self._cell_angles[0:3] = converter([float(line[36:48]),
            float(line[48:60]), float(line[60:72])])
   ...
   def coordinates(self, stuff):
      ...
      # Try to flatten the array if we got a 3-D numpy array. Don't worry if it
      # doesn't work, just go on
      try:
         stuff = stuff.flatten()
      except AttributeError:
         pass
      if self.natom > 0 and len(stuff) != 3 * self.natom:
         raise ValueError('Only got %d coordinates for %d atoms' % (len(stuff),
            self.natom))
      if len(stuff) % 3 != 0:
         raise ValueError('Number of coordinates (%d) is not a multiple of 3' %
               len(stuff))
      if self._coords_written:
         raise RuntimeError('Coordinates have already been written.')
      # Error checking done. If we didn't already set our number of atoms, set
      # that now
   ...
   def velocities(self, stuff):
      ...
      if len(stuff) != 3 * self.natom:
         raise ValueError('Got %d velocities for %d atoms.' % (len(stuff),
            self.natom))
      ...
      for i in range(self.natom):
         ...
         self._file.write(fmt % (stuff[i3] * ONEVELSCALE,
            stuff[i3 + 1] * ONEVELSCALE, stuff[i3 + 2] * ONEVELSCALE))
         numwrit += 1
         if numwrit % 2 == 0: self._file.write('\n')
      if self.natom % 2 == 1: self._file.write('\n')
...
class AmberMdcrd(_AmberAsciiCoordinateFile):
   ...
   def _parse(self):
      """
      This method parses the data out of the mdcrd file and creates self.data as
      a list of np.ndarray(natom*3) (or list of array.array(natom*3) if numpy is
      not available) and self.cell_lengths as a list of np.ndarray(3) (or list
      of array.array(3) if numpy is not available) for each frame in the
      trajectory. This method is called automatically for 'old' trajectory files
      and should not be called by external callers.
      """
      self.frame = 0
      ...
      try:
         ...
         _warnings.warn('Unexpected EOF in parsing mdcrd. natom and/or hasbox '
                        'are likely wrong', RuntimeWarning)
