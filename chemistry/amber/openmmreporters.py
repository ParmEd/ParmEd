...
class AmberStateDataReporter(object):
   ...
   Most of this code is copied from the OpenMM StateDataReporter class, with the
   above-mentioned changes made.
   """
   ...
   def __init__(self, f, reportInterval, step=True, time=True,
                ...
                densityUnit=u.grams/u.item/u.milliliter, timeUnit=u.picosecond):
      """ Create a StateDataReporter.
      Parameters:
       - f (string or writable object) The object to write to, specified as a
         file name or writable object
       - reportInterval (int) The interval (in time steps) at which to write
         frames
       - step (boolean=False) Whether to write the current step index
       - time (boolean=False) Whether to write the current time
       ...
       - systemMass (mass=None) The total mass to use for the system when
         reporting density.  If this is None (the default), the system mass is
         computed by summing the masses of all particles.  This parameter is
         useful when the particle masses do not reflect their actual physical
         mass, such as when some particles have had their masses set to 0 to
         immobilize them.
      """
   ...
   def _constructReportValues(self, simulation, state):
      ...
      if self._time:
         values.append(state.getTime().value_in_unit(self.timeUnit))
      if self._potentialEnergy:
         values.append(state.getPotentialEnergy().value_in_unit(self.energyUnit))
      if self._kineticEnergy:
         values.append(state.getKineticEnergy().value_in_unit(self.energyUnit))
      if self._totalEnergy:
         totEne = state.getKineticEnergy() + state.getPotentialEnergy()
         values.append(totEne.value_in_unit(self.energyUnit))
      if self._temperature:
         values.append((2*state.getKineticEnergy()/(self._dof*u.MOLAR_GAS_CONSTANT_R)).value_in_unit(self.temperatureUnit))
      if self._volume:
         values.append(volume.value_in_unit(self.volumeUnit))
      if self._density:
         values.append((self._totalMass/volume).value_in_unit(self.densityUnit))
      return values
   ...
   def _initializeConstants(self, simulation):
      ...
      if self._temperature:
         ...
         if any(type(frc) == mm.CMMotionRemover for frc in system.getForces()):
            dof -= 3
         self._dof = dof
      if self._density:
...
class NetCDFReporter(object):
   ...
   def __init__(self, file, reportInterval, atom, uses_pbc,
      ...
      if not crds and not vels and not frcs:
         raise ValueError('You must print either coordinates, velocities, or '
                          'forces in a NetCDFReporter')
      # Control flags
   ...
   def describeNextReport(self, simulation):
      ...
      Returns:
         A five element tuple.  The first element is the number of steps until
         the next report.  The remaining elements specify whether that report
         will require positions, velocities, forces, and energies respectively.
      """
      steps = self._reportInterval - simulation.currentStep%self._reportInterval
   ...
   def report(self, simulation, state):
      ...
      if self._out is None:
         ...
         self._out = NetCDFTraj.open_new(self.fname, self.atom,
               self.uses_pbc, self.crds, self.vels, self.frcs,
               title="ParmEd-created trajectory using OpenMM")
      if self.uses_pbc:
         lengths, angles = _process_box_vectors(*state.getPeriodicBoxVectors())
...
class MdcrdReporter(object):
   ...
   def report(self, simulation, state):
      ...
      if self.vels:
         vels = state.getVelocities().value_in_unit(VELUNIT)
         # Divide by the scaling factor (works if vels is a list of Vec3's) This
         # is necessary since AmberMdcrd does not scale before writing (since it
         # expects coordinates)
...
class RestartReporter(object):
   ...
   def __init__(self, file, reportInterval, natom, uses_pbc,
      ...
      Parameters:
         ...
         -  write_multiple (bool): Write a new restart file (named with a .# on
               top of the file name) each time a restart is written?
...
def _process_box_vectors(a, b, c):
   """
   This converts 3 periodic box vectors, converting to units of angstroms, and
   returns a 2-element tuple of box lengths and box angles. Both will be
...
class ProgressReporter(AmberStateDataReporter):
   ...
   def __init__(self, f, reportInterval, totalSteps, potentialEnergy=True,
      ...
      """ Create a StateDataReporter.
      Parameters:
       - f (string) The file name of the progress report (overwritten each time)
       - reportInterval (int) The interval (in time steps) at which to write
         frames
      ...
      """
      # Make sure we got a file name rather than a file-like object. Immediately
      # close the file after opening. This erases it, which isn't a bad thing,
      # but also prepares it for reports
      super(ProgressReporter, self).__init__(f, reportInterval, step=True,
            time=True, potentialEnergy=potentialEnergy,
   ...
   def report(self, simulation, state):
      """
      Generate a report and predict the time to completion (and current/overall
      MD performance)
      """
      if not self._hasInitialized:
         self._initializeConstants(simulation)
   ...
   def _constructReportValues(self, simulation, state):
      ...
      if self._kineticEnergy:
         values['kineticEnergy'] = kinetic.value_in_unit(self.energyUnit)
      if self._totalEnergy:
         values['totalEnergy'] = \
               values['potentialEnergy'] + values['kineticEnergy']
      if self._temperature:
         values['temperature'] = (2 * kinetic /
               (self._dof * u.MOLAR_GAS_CONSTANT_R)
         ).value_in_unit(self.temperatureUnit)
      if self._volume:
         values['volume'] = volume.value_in_unit(self.volumeUnit)
      if self._density:
         values['density'] = (self._totalMass / volume).value_in_unit(
               self.densityUnit)
...
class EnergyMinimizerReporter(AmberStateDataReporter):
   """
   This class acts as a simple energy reporter class for minimizations. This is
   not meant to be used as a reporter for OpenMM's molecular dynamics routines,
   but instead passed a simulation object for printing out single-point
   energies.
   """
   ...
   def describeNextReport(self, *args, **kwargs):
      ...
      raise NotImplemented('EnergyMinimizerReporter is not intended to be used '
                           'for reporting on molecular dynamics')
   ...
   def report(self, simulation, frame=None):
      ...
      if has_pbc and (self._volume or self._density):
         box = state.getPeriodicBoxVectors()
         volume = (box[0][0]*box[1][1]*box[2][2]).value_in_unit(self.volumeUnit)
      if has_pbc and self._volume:
         self._out.write('   Volume = %12.4f %s\n' % (volume, self.volumeUnit))
      self._out.write('\n')
