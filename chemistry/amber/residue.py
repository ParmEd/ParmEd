...
AMINO = ["ALA", "ARG", "ASH", "ASN", "ASP", "CYM", "CYS", "CYX", "GLH", "GLN",
         "GLU", "GLY", "HID", "HIE", "HIP", "ILE", "LEU", "LYN", "LYS", "MET",
...
def ToResidue(molecule, resnum):
    ...
    try:
        # 1 after the end, as it needs to be for array slices
        end = molecule.residue_pointers[resnum+1]
    except IndexError:
        # This must be the last residue
        end = len(molecule.atoms)

    # Gather the bonding information for the given residue. Create a new bond
    # array that is specific to this residue (and whose partners are indexed as
    # though this residue starts from atom 0). Also find the "connect" atoms
    # that bond to adjacent residues.
    bonds = []
    connects = [0,0]
    head = tail = 0
        ...
        for j in molecule.bonds[i]:
            if j < begin and head == 0:
                ...
                if molecule.residue_container[j] == resnum + 1:
                    # we keep track of index in connects and the atom number so
                    # we can remove it from here and replace it in connects[1]
                    # at the end.  we do this so that we only count the last
                    # "tail" residue as connect1
                    tail = len(connects)
                    connects.append(i - begin + 1)
                else:
                    connects.append(i - begin + 1)
            else:
                # otherwise it's an intra-residue bond. adjust j for new
                # numbering and add it to bonds
    ...
    return Residue(atoms=molecule.atoms[begin:end],
                   atom_types=molecule.atom_types[begin:end],
                   charges=molecule.charges[begin:end],
                   bonds=bonds, coords=molecule.coords[3*begin:3*end],
                   head=head, tail=tail, connects=connects,
                   resname=molecule.residues[resnum],
...
class Residue(object):
    ...
    def OFF(self):
        """
        Returns a string that is this residue/unit's entry in OFF file format.
        For OFF file format docs see http://ambermd.org/doc/OFF_file_format.txt
        """

        # Write out the atoms table section
        ret_string = (
                "!entry.%s.unit.atoms table  str name  str type  int typex "
        ...
        ret_string += ('!entry.%s.unit.hierarchy table  str abovetype  '
        ...
        ret_string += ("!entry.%s.unit.residueconnect table  int c1x  "
        ...
        if self.orig_name in AMINO:
            attype = 'p'
        elif self.orig_name in NUCLEIC:
            attype = 'n'
        elif self.orig_name == 'WAT':
            attype = 'w'
        else:
            attype = '?'
        ret_string += ("!entry.%s.unit.residues table  str name  int seq  "
                       "int childseq  int startatomx  str restype  int "
                       "imagingx\n" % self.name
        )
    ...
    def __eq__(self, other):
        ...
        if ((self.head == 0 and other.head != 0) or
            (self.head != 0 and other.head == 0)):
            return False

        if ((self.tail == 0 and other.tail != 0) or
            (self.tail != 0 and other.tail == 0)):
            return False
