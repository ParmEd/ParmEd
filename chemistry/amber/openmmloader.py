...
class OpenMMAmberParm(AmberParm):
   ...
   def openmm_LJ(self):
      ...
      for i in range(self.pointers["NTYPES"]):
         ...
         if self.parm_data["LENNARD_JONES_ACOEF"][lj_index] < 1.0e-10:
            ...
            LJ_depth.append(self.parm_data["LENNARD_JONES_BCOEF"][lj_index]
                                    / 2 / factor)
      ...
      for i in range(ntyp):
         for j in range(ntyp):
            ...
            acoef = self.parm_data['LENNARD_JONES_ACOEF'][idx]
            bcoef = self.parm_data['LENNARD_JONES_BCOEF'][idx]
      ...
      if self.ptr('ifbox'):
         if hasattr(self, 'rst7'):
            self._topology.setUnitCellDimensions(
               tuple(x.value_in_unit(u.nanometer) for x in self.rst7.box[:3]
                     *u.angstrom)*u.nanometer)
         else:
            self._topology.setUnitCellDimensions(
               tuple(x.value_in_unit(u.nanometer) for x in
               self.parm_data['BOX_DIMENSIONS'][1:4]*u.angstrom)*u.nanometer)
   ...
   def createSystem(self, nonbondedMethod=ff.NoCutoff,
      ...
      """
      # Rebuild the topology file if necessary, and flush the atom property data
      # to the atom list
      ...
      for angle in self.angles_inc_h:
         ...
         if constrained:
            ...
            length = sqrt(l1*l1 + l2*l2 - 2*l1*l2*cos(angle.angle_type.theteq))
            system.addConstraint(bond.atom1.starting_index,
      ...
      for angle in self.angles_without_h:
         force.addAngle(angle.atom1.starting_index, angle.atom2.starting_index,
                        angle.atom3.starting_index, angle.angle_type.theteq,
                        2*angle.angle_type.k*angle_frc_conv)
      system.addForce(force)
      # Add dihedral forces
      ...
      for tor in self.dihedrals_inc_h + self.dihedrals_without_h:
         force.addTorsion(tor.atom1.starting_index, tor.atom2.starting_index,
                          tor.atom3.starting_index, tor.atom4.starting_index,
                          int(tor.dihed_type.per), tor.dihed_type.phase,
      ...
      if self.ptr('ifbox') == 0: # non-periodic
         ...
         # Set up box vectors (from inpcrd if available, or fall back to prmtop
         # definitions
         ...
         if cutoff is None:
            ...
            min_box_width = min((box[0] / u.nanometers, box[1] / u.nanometers,
                                 box[2] / u.nanometers))
      ...
      if len(ep) > 0:
         ...
         for bond in self.bonds_inc_h + self.bonds_without_h:
            if bond.atom1.residue.has_ep:
               res = bond.atom1.residue.idx
               if bond.atom1.element == 1 or bond.atom2.element == 1:
                  ...
                  if bond.atom1.element == 8 or bond.atom2.element == 8:
                     distOH[res] = bond.bond_type.req * u.angstroms
               elif ((bond.atom1.element == 8 or bond.atom2.element == 8) and
                     (bond.atom1.element == 0 or bond.atom2.element == 0)):
         ...
         for residue in self.residue_list:
            ...
            if len(waterO[res]) == 1 and len(waterH[res]) == 2:
               if len(waterEP[res]) == 1:
                  ...
                  system.setVirtualSite(waterEP[res][0],
               ...
               # 5-point water
               weightH = cosOOP * distOE[res] / sqrt(distOH[res] * distOH[res] -
                                 0.25 * distHH[res] * distHH[res])
      ...
      if implicitSolvent is not None:
         ...
         # If implicitSolventKappa is None, compute it from salt concentration
         if implicitSolventKappa is None:
            ...
            # sander/pmemd's kappa conversion factor)
         ...
         system.addForce(gb)
         force.setReactionFieldDielectric(1.0) # applies to NonbondedForce above
      ...
      if hydrogenMass is not None:
         for bond in self.bonds_inc_h:
            ...
            if atom1.element != 1:
               ...
               new_mass1 = system.getParticleMass(atom1.index) - transfer_mass
   ...
   def box_vectors(self):
      ...
      if hasattr(self, 'rst7'):
         ...
         return _box_vectors_from_lengths_angles(box[0], box[1], box[2], ang[0],
                        ang[1], ang[2])
      else:
