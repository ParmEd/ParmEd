...
class Atom(object):
    ...
    def load_from_parm(self):
        ...
        self.atomic_number = parm_data['AMOEBA_ATOMIC_NUMBER'][si]
        ...
        self.typeidx = parm_data['AMOEBA_ATOM_TYPE_INDEX'][si]
        self.classidx = parm_data['AMOEBA_ATOM_CLASS_INDEX'][si]
        self.nb_idx = parm_data['AMOEBA_VDW_ATOM_TYPES_LIST'][si]
        self._vdw_par = parm_data['AMOEBA_VDW_ATOM_PARENT_LIST'][si]
        self.vdw_par_wt = parm_data['AMOEBA_VDW_PARENT_COORD_WEIGHT_LIST'][si]
        self.multipoles = parm_data['AMOEBA_LOCAL_FRAME_MULTIPOLES_LIST'] \
                                                [10*si:10*si+10]
        self.polar = parm_data['AMOEBA_POLARIZABILITY_LIST'][si]
    ...
    def add_data(self):
        ...
        data['ATOM_NAME'][idx] = self.atname[:4]
        data['NUMBER_EXCLUDED_ATOMS'][idx] = numex
        data['CHARGE'][idx] = 0.0
        data['MASS'][idx] = self.mass
        data['TREE_CHAIN_CLASSIFICATION'][idx] = self.tree
        data['ATOM_TYPE_INDEX'][idx] = 1 # hard-coded
        data['AMBER_ATOM_TYPE'][idx] = self.attype
        data['AMOEBA_ATOMIC_NUMBER'][idx] = self.atomic_number
        data['AMOEBA_ATOM_TYPE_INDEX'][idx] = self.typeidx
        data['AMOEBA_ATOM_CLASS_INDEX'][idx] = self.classidx
        data['AMOEBA_VDW_ATOM_TYPES_LIST'][idx] = self.nb_idx
    ...
    def tortor_to(self, other):
        ...
        self._tortor_partners.add(other)
    ...
    def determine_all_exclusion_partners(self, first_excl):
        ...
        This method looks through the exclusion list and adds any atoms that
        are not naturally accounted for by the bond, angle, torsion, and
    ...
    def determine_exclusions_from_bonds(self):
        ...
        exclusion list
        """
        for a1 in self.bond_partners:
            # Now loop through every bond of every connected atom, and make
            # sure it is part of the angle partners
            for a2 in a1.bond_partners:
                # All of these atoms are angle partners
                if a2 is not self:
                    self._angle_partners.add(a2)
                for a3 in a2.bond_partners:
                    # All of these atoms are dihedral partners
                    if a3 is not self:
                        self._dihedral_partners.add(a3)
                    for a4 in a3.bond_partners:
                        # All of these atoms are tortor partners
                        if a4 is not self:
                            self._tortor_partners.add(a4)
    ...
    def exclusion_partners(self):
        return sorted(list(self._exclusion_partners - self._tortor_partners -
                           self._dihedral_partners - self._angle_partners -
                           self._bond_partners))
...
class AtomList(list):
    ...
    def __init__(self, parm, fill_from=None):
        ...
        # every atom exists before we try to assign the parent atoms (since we
        # can't be assured that the necessary atom will already exist if we
        # assign this at list construction time)
    ...
    def _determine_exclusions(self):
        ...
        # The Amoeba force field does not have lone pairs. No need to determine
        # EP exclusions here like we did in topologyobjects.py
        for atm in self:
            vals_to_add = []
            ...
            for member in atm.dihedral_partners:
                if member.idx > atm.idx: vals_to_add.append(member.idx+1)
            for member in atm.tortor_partners:
                if member.idx > atm.idx: vals_to_add.append(member.idx+1)
...
class UreyBradleyType(_UreyBradleyType):
    ...
    def write_info(self, parm):
        ...
        data = parm.parm_data
        data['AMOEBA_UREY_BRADLEY_BOND_FORCE_CONSTANT'][self.idx] = self.k
        data['AMOEBA_UREY_BRADLEY_BOND_EQUIL_VALUE'][self.idx] = self.req
...
class UreyBradleyTypeList(_TypeList):
    ...
    def _make_array(self):
        ...
        list.__init__(self, [UreyBradleyType(k, eq, -1)
                             for k, eq in zip(kl, eql)])

    def write_to_parm(self):
        data = self.parm.parm_data
        data['AMOEBA_UREY_BRADLEY_BOND_FTAB_DEGREE'][0] = self.degree
        data['AMOEBA_UREY_BRADLEY_BOND_FTAB_COEFFS'] = self.coeffs[:]
...
class AngleType(_AngleType):
    ...
    def write_info(self, parm):
        ...
        if self.idx == -1: return
        data = parm.parm_data
        data['AMOEBA_REGULAR_ANGLE_FORCE_CONSTANT'][self.idx] = self.k
        data['AMOEBA_REGULAR_ANGLE_EQUIL_VALUE'][self.idx] = self.theteq
...
class AngleTypeList(_TypeList):
    ...
    def write_to_parm(self):
        data = self.parm.parm_data
        data['AMOEBA_REGULAR_ANGLE_FTAB_DEGREE'][0] = self.degree
        data['AMOEBA_REGULAR_ANGLE_FTAB_COEFFS'] = self.coeffs[:]
...
class TrigonalAngleType(_AngleType):
    ...
    def write_info(self, parm):
        ...
        if self.idx == -1: return
        data = parm.parm_data
        data['AMOEBA_TRIGONAL_ANGLE_FORCE_CONSTANT'][self.idx] = self.k
        data['AMOEBA_TRIGONAL_ANGLE_EQUIL_VALUE'][self.idx] = self.theteq
...
class TrigonalAngleTypeList(_TypeList):
    ...
    def write_to_parm(self):
        data = self.parm.parm_data
        data['AMOEBA_TRIGONAL_ANGLE_FTAB_DEGREE'][0] = self.degree
        data['AMOEBA_TRIGONAL_ANGLE_FTAB_COEFFS'] = self.coeffs[:]
...
class DihedralTypeList(_TypeList):
    ...
    def _make_array(self):
        ...
        phasel = self.parm.parm_data['AMOEBA_TORSION_PHASE']
        list.__init__(self, [DihedralType(k, per, phase, -1)
                             for k, per, phase in zip(kl, perl, phasel)])
...
class PiTorsionType(DihedralType):
    ...
    def write_info(self, parm):
        if self.idx == -1: return
        data = parm.parm_data
        data['AMOEBA_PI_TORSION_FORCE_CONSTANT'][self.idx] = self.phi_k
        data['AMOEBA_PI_TORSION_PERIODICITY'][self.idx] = self.per
        data['AMOEBA_PI_TORSION_PHASE'][self.idx] = self.phase
...
class PiTorsionTypeList(_TypeList):
    ...
    def _make_array(self):
        ...
        phasel = self.parm.parm_data['AMOEBA_PI_TORSION_PHASE']
...
class StretchBendType(object):
    ...
    def write_info(self, parm):
        if self.idx == -1: return
        data = parm.parm_data
        data['AMOEBA_STRETCH_BEND_FORCE_CONSTANT'][self.idx] = self.k
        data['AMOEBA_STRETCH_BEND_ANGLE_EQUIL_VALUE'][self.idx] = self.theteq
        data['AMOEBA_STRETCH_BEND_BOND1_EQUIL_VALUE'][self.idx] = self.bond1eq
        data['AMOEBA_STRETCH_BEND_BOND2_EQUIL_VALUE'][self.idx] = self.bond2eq
...
class TorsionTorsionType(object):
    ...
    def __init__(self, dims, ang1, ang2, f, dfda1, dfda2, d2fda1da2, idx):
        ...
        Parameters:
            ...
            ang1 (list of floats): Angles in 1st dim of interpolation table
            ang2 (list of floats): Angles in 2nd dim of interpolation table
            ...
            d2fda1da2 (list of floats): 2nd derivative of f w.r.t. both angles
            idx (int): Index of this type
        """

        self.dims = tuple(dims)
        if self.dims[0] != len(ang1):
            raise AmoebaError('Mismatched torsion-torsion dimensions '
                              '[%d; got %d]' % (self.dims[0], len(ang1)))
        if self.dims[1] != len(ang2):
            raise AmoebaError('Mismatched torsion-torsion dimensions '
                              '[%d; got %d]' % (self.dims[1], len(ang2)))
    ...
    def write_info(self, parm, after):
        if self.idx == -1: return
        # We need to get rid of the existing sections in the prmtop and
        # overwrite it with the data here
        i = self.idx + 1
