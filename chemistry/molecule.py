...
class Molecule:
    ...
    def __init__(self, atoms=[], atom_types=[], charges=[], residues=[],
                 bonds=[], residue_pointers=[], coords=[], elements=[],
                 title='',radii=[]):
    ...
    def DeleteAtom(self, atomno):
        ...
        # Get rid of now-vacant residues if we removed the last atom from that
        # residue
    ...
    def _check(self):
        ...
        if len(self.charges) != len(self.atoms):
            raise(MoleculeError('len(atoms) != len(charges)'))

        if len(self.coords) != 3 * len(self.atoms):
            raise(MoleculeError('len(atoms) != len(coords) * 3'))

        if len(self.residue_pointers) != len(self.residues):
            raise(MoleculeError('len(residue_pointers) != len(residues)'))

        if len(self.elements) != len(self.atoms):
            raise(MoleculeError('len(elements) != len(atoms)'))
