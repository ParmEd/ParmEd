...
class Atom(_BaseParticle):
   ...
   def __str__(self):
      return ('Atom %d [%s]:\n\tAtomic Number: %d\n\tTemp Factor:   %.2f\n\t'
              ...
              self.atomic_number, self.bfactor, self.occupancy, self.x, self.y,
              self.z, self.altloc)
      )
...
class Residue(_BaseParticle, list):
   ...
   def __eq__(self, other):
      """
      Two residues are the same if their name, number, insertion code, and chain
      are the same
      """
      if self.name != other.name: return False
      if self.number != other.number: return False
   ...
   def __str__(self):
      return ('Residue:\n\tName:   %s\n\tNumber: %s\n\tChain:  %s\n\tInsCod: %s'
               % (self.name, self.number, self.chain, self.insertion_code))

   def __repr__(self):
      return ('<Residue %d (%s); Chain %s; InsCode %s>' % (self.number,
                  self.name, self.chain, self.insertion_code))
   ...
   def first_atoms(self):
      ...
      for atom in self:
         # Require consective atoms to be sequential (as they always are in the
         # PDB)
   ...
   def load_from_open_pdb(cls, pdb):
      ...
      for line in pdb:
         ...
         if rec == 'ATOM  ' or rec == 'HETATM':
            ...
            inscode, x, y, z = line[26], line[30:38], line[38:46], line[47:54]
            ...
            try:
               atomic_number = _AtomicNum[(elem[0] + elem[1].lower()).strip()]
            except KeyError:
               atomic_number = -1
            inst.add_atom(
               Atom(atnum, atname, atomic_number, chg, x=x, y=y, z=z,
                    occupancy=occupancy, bfactor=bfactor, altloc=altloc),
               Residue(resname, resid, inscode, chain, inst.models or 1),
            )
         if rec == 'MODEL ':
