...
def positional_restraints(mask, weights, refc, scriptfile=None):
   ...
   frc = mm.CustomExternalForce('k*((x-x0)*(x-x0)+(y-y0)*(y-y0)+(z-z0)*(z-z0))')
...
def simulate(parm, args):
   ...
   if mdin.cntrl_nml['ilscivr']:
      raise SimulationError('Linearized semi-classical simulations are not '
                            'supported.')
   if parm.ptr('ifbox') > 1:
      raise SimulationError('OpenMM currently supports only orthorhombic cells')
   ...
   if abs(mdin.cntrl_nml['ntwr']) < 500:
      warnings.warn('ntwr is set quite low (%d). Consider much larger values '
            'to improve computational performance.' % mdin.cntrl_nml['ntwr'],
            SimulationWarning)
   ...
   if (mdin.cntrl_nml['ntb'] == 2 or mdin.cntrl_nml['ntp'] > 0) and runmd:
      ...
      if mdin.cntrl_nml['barostat'] == 1:
         raise SimulationError('Berendsen barostat is not implemented in '
               'OpenMM. Use the MC barostat instead (barostat=2)')
   ...
   if mdin.cntrl_nml['ntt'] == 3:
      ...
      if scriptfile is not None:
         scriptfile.write(
               'integrator = LangevinIntegrator(%s*u.kelvin, %s/u.picosecond,\n'
               '                    %s*u.picosecond\n)\n' %
      ...
      if mdin.cntrl_nml['ntt'] == 1:
         ...
         if scriptfile is not None:
            scriptfile.write('system.addForce(AndersenThermostat(%s*u.kelvin,\n'
                             '                %s\n)\n' %
                             (mdin.cntrl_nml['temp0'], mdin.cntrl_nml['vrand']))
         system.addForce(mm.AndersenThermostat(mdin.cntrl_nml['temp0']*u.kelvin,
                              mdin.cntrl_nml['vrand'])
         )
      if scriptfile is not None:
   ...
   # chosen platform. To handle the case where we chose the default platform, we
   # need to get the platform from the current context
   if platform is None:
   ...
   if inpcrd is not None:
      ...
      if position_container.natom != parm.ptr('natom'):
         raise SimulationError('inpcrd [%s] and prmtop mismatch in number of '
                               'atoms' % inpcrd)
   else:
   ...
   if mdin.cntrl_nml['imin'] > 0:
      ...
      if mdin.cntrl_nml['imin'] == 1:
         ...
         restrt_reporter = RestartReporter(restart, 1, parm.ptr('natom'),
               mdin.cntrl_nml['ntb'] > 0, False, mdin.cntrl_nml['ntxo'] == 2,
         ...
         if mdin.cntrl_nml['ioutfm'] == 0:
            ...
            if scriptfile is not None:
               scriptfile.write('crd_reporter = (\n'
                     ...
                     '         vels=False, frcs=False)\n)\n' %
                     (trajectory, "parm.ptr('natom')", mdin.cntrl_nml['ntb']>0))
         elif dcd:
   ...
   # Otherwise we want to do dynamics
         ...
         if mdin.cntrl_nml['ioutfm'] == 0:
            simulation.reporters.append(
                  MdcrdReporter(trajectory, mdin.cntrl_nml['ntwx'],
                     parm.ptr('natom'), mdin.cntrl_nml['ntb'] > 0, crds=True,
                     vels=False, frcs=False)
            )
            if scriptfile is not None:
               scriptfile.write('simulation.reporters.append(\n'
                  ...
                  '         vels=False, frcs=False)\n'
                  ')\n' %
                  (trajectory, mdin.cntrl_nml['ntwx'], "parm.ptr('natom')",
                     mdin.cntrl_nml['ntb'] > 0)
               )
         elif dcd:
            simulation.reporters.append(
            ...
            if scriptfile is not None:
               scriptfile.write('simulation.reporters.append(\n'
                  ...
                  '      )\n)\n' %
                  (trajectory, mdin.cntrl_nml['ntwx'], "parm.ptr('natom')",
                     mdin.cntrl_nml['ntb'] > 0, mdin.cntrl_nml['ntwv'] < 0,
                     mdin.cntrl_nml['ntwf'] < 0)
               )
         ...
         if mdin.cntrl_nml['ioutfm'] == 0:
            simulation.reporters.append(
                  MdcrdReporter(mdvel, mdin.cntrl_nml['ntwv'],
                     parm.ptr('natom'), mdin.cntrl_nml['ntb'] > 0, crds=False,
            ...
            if scriptfile is not None:
               scriptfile.write('simulation.reporters.append(\n'
                  ...
                  '         vels=True, frcs=False)\n'
                  ')\n' %
                  (trajectory, mdin.cntrl_nml['ntwv'], "parm.ptr('natom')",
                     mdin.cntrl_nml['ntb'] > 0)
               )
         elif dcd:
            # Should never make it here due to earlier checks; catch it anyway
            raise SimulationError('Cannot write velocities to DCD files.')
         else:
            # Add forces too only if ntwf < 0 AND no coordinate traj is being
            # printed (otherwise forces will be printed to the coordinate file
            _frcs = mdin.cntrl_nml['ntwf'] < 0 and mdin.cntrl_nml['ntwx'] == 0
            simulation.reporters.append(
                  NetCDFReporter(mdvel, mdin.cntrl_nml['ntwv'],
                     parm.ptr('natom'), mdin.cntrl_nml['ntb'] > 0, crds=False,
                     vels=True, frcs=_frcs)
            )
            if scriptfile is not None:
               scriptfile.write('simulation.reporters.append(\n'
                  '      NetCDFReporter("%s", %s, %s, %s,\n'
         ...
         if mdin.cntrl_nml['ioutfm'] == 0:
            ...
            if scriptfile is not None:
               scriptfile.write('simulation.reporters.append(\n'
                  ...
                  '         vels=False, frcs=True)\n'
                  ')\n' %
                  (trajectory, mdin.cntrl_nml['ntwf'], "parm.ptr('natom')",
                     mdin.cntrl_nml['ntb'] > 0)
               )
         elif dcd:
            # Should never make it here due to earlier checks; catch it anyway
      ...
      if runsim:
         ...
         # Now write the final restart file, hijacking the restart file reporter
         final_state = simulation.context.getState(getPositions=True,
               getVelocities=True, enforcePeriodicBox=mdin.cntrl_nml['ntb'] > 0)
         restrt_reporter.report(simulation, final_state)
         timer.stop_timer('md')
         nsperday = (mdin.cntrl_nml['nstlim'] * mdin.cntrl_nml['dt'] / 1000.0 /
                     (timer.timers['md'] / 3600 / 24))
...
def energy(parm, args, output=sys.stdout):
   ...
   if decomp:
      ...
      bond = state.getPotentialEnergy().value_in_unit(u.kilocalories/u.mole)
      # Now angles
      state = context.getState(getEnergy=True, enforcePeriodicBox=has_pbc,
                               groups=2**parm.ANGLE_FORCE_GROUP)
      angle = state.getPotentialEnergy().value_in_unit(u.kilocalories/u.mole)
      # Now dihedrals
      state = context.getState(getEnergy=True, enforcePeriodicBox=has_pbc,
      ...
      output.write('Potential Energy = %.7f kcal/mol\n' %
               state.getPotentialEnergy().value_in_unit(u.kilocalories/u.mole))
