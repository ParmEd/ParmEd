...
class Action(lawsuit):
   ...
   def __init__(self, input_parm, arg_list=None, *args, **kwargs):
      ...
      # If our arg_list is a string, convert it to an ArgumentList (but coerce
      # to string if it is string-like)
      try:
...
class changelj14pair(Action):
   ...
   def init(self, arg_list):
      ...
      if not self.parm.chamber:
         raise ChangeLJPairError('Changing 1-4 NB pairs makes no sense for non-'
                                 'chamber created prmtops!')
      ...
      if self.prop in ('CHARGE', 'RADII', 'SCREEN', 'MASS'):
         ...
         if len(self.new_val) > 4:
            warnings.warn('Only 4 letters allowed for %s entries!'
                   'Truncating remaining letters.' % self.prop, ParmWarning)
         ...
         if sel == 1:
            if attype is None:
               ...
               if attype != self.parm.parm_data['ATOM_TYPE_INDEX'][i]:
                  raise LJ_TypeError('changeLJSingleType: Selection mask has '
                                     'multiple atom types!')
...
class setmolecules(Action):
   ...
   def init(self, arg_list):
      ...
      if solute_ions is None:
         ...
         warnings.warn("Value of solute_ions is unrecognized [%s]! Assuming "
                       "True" % solute_ions, SeriousParmWarning)
         self.solute_ions = True
   ...
   def execute(self):
      ...
      if owner is not None:
         if not hasattr(self.parm, 'rst7'):
            warnings.warn('The atoms in %s were reordered to correct molecule '
                  'ordering. Any topology printed from now on will _not_ work '
                  'with the original inpcrd or trajectory files created with '
                  'this prmtop! Consider quitting and loading a restart prior '
                  'to using setMolecules' % self.parm, ParmWarning)
...
class adddihedral(Action):
   ...
   def execute(self):
      ...
      for it in range(sum(sel1)):
         ...
         dihed_list.append(Dihedral(atm1, atm2, atm3, atm4, new_dih_typ, signs))
      ...
      for i in range(sum(sel1)):
         ...
         if (atm1 == atm2 or atm1 == atm3 or atm1 == atm4 or 
            ...
            warnings.warn('Skipping %d-%d-%d-%d dihedral deletion -- '
                          'duplicate atoms!'% (atnum1, atnum2, atnum3, atnum4),
                          SeriousParmWarning)
            continue
      ...
      for ty in sel_types:
         for ty2 in range(1,ntypes+1):
            ...
            ret_str += '%%%ds %%%ds %%15.6f %%15.6f %%10.6f %%10.6f\n' % (
                     maxlen, maxlen) % (typenames[type1-1], typenames[type2-1],
                        acoef, bcoef, rij, eij)
...
class interpolate(Action):
   ...
   def init(self, arg_list):
      ...
      if parm2 is None and len(self.parm_list) == 2:
         ...
         raise AmbiguousParmError('You must identify parm2 if more than 2 parm '
                                  'instances exist!')
      else:
         try:
            parm2 = int(parm2)
   ...
   def __str__(self):
      ...
      return 'Creating %d interpolated prmtops between %s and %s' % (self.nparm,
                  self.parm, self.parm2) + extra
   ...
   def _check_parms(self):
      ...
      if parm1.ptr('natom') != parm2.ptr('natom'):
         raise IncompatibleParmsError('%s and %s have different #s of atoms!' %
                                      (parm1, parm2))
      ndiff = 0
      for i in range(parm1.ptr('natom')):
         if parm1.parm_data['ATOM_NAME'][i] != parm2.parm_data['ATOM_NAME'][i]:
            ndiff += 1
      if ndiff > 0:
         warnings.warn('%d atoms have different names b/w %s and %s' % (ndiff,
                        parm1, parm2), SeriousParmWarning)
      for atm1, atm2 in zip(parm1.atom_list, parm2.atom_list):
         if (parm1.LJ_radius[atm1.nb_idx-1] != parm2.LJ_radius[atm2.nb_idx-1] or
             parm1.LJ_depth[atm1.nb_idx-1] != parm2.LJ_radius[atm2.nb_idx-1]):
            self.diff_vdw = True
...
class addpdb(Action):
   ...
   The 'strict' keyword turns residue mismatches (excluding solvent) into errors
   ...
   Historical info:
      This action is based on, and reproduces the key results of, the historical
      'add_pdb' program. It is a bit more flexible, though.
   """
   ...
   def execute(self):
      ...
      for i, res in enumerate(pdb):
         try:
            ...
            if res.name.strip() != reslab:
               if (not res.name.strip() in ('WAT', 'HOH') or 
                   not reslab in ('WAT', 'HOH')):
                  # Allow for 3's and 5's in terminal nucleic acid residues
                  if res.name.strip() in ('A','C','G','U','DA','DG','DC','DT'):
                     if reslab[-1] in '35':
                        reslab = reslab[:-1]
                  if reslab != res.name.strip():
                     warnings.warn('Residue name mismatch [#%d] %s vs. %s' %
                              (i+1, res.name.strip(), reslab), AddPDBWarning)
      ...
      self.parm.addFlag('RESIDUE_NUMBER', '20I4', data=resnums, comments=ncmts)
      self.parm.addFlag('RESIDUE_CHAINID', '20a4', data=chainids,
            comments=['Residue chain ID (chainId) read from PDB file; '
                      'DIMENSION(NRES)']
      )
      if haveicodes:
...
class add12_6_4(Action):
   ...
   Lennard-Jones potential term. If provided, the mask will allow you to specify
   which ions are divalent. The C4 parameter between the metal ion and water 
   can either be taken from Ref. [1] for the requested [watermodel] (TIP3P, 
   TIP4PEW, or SPCE) or provided in the file specified by the c4file keyword.
   The polarizabilities must be present in the the polfile file. The chemical 
   symbol of the element will be used to determine the atom type. Parameters 
   are expected in a file with 2 columns:
     <atom type>   <parameter>
   ...
   def execute(self):
      ...
      for i, atom in enumerate(self.parm.atom_list):
         ...
         if heteroatom is None:
            ...
            warnings.warn('H atom detected not bound to heteroatom. Ignoring.',
                          ParmWarning)
            continue
         transfermass = self.new_h_mass - self.parm.parm_data['MASS'][i]
         self.parm.parm_data['MASS'][i] = self.new_h_mass
         self.parm.parm_data['MASS'][heteroatom.starting_index] -= transfermass

      # Now make sure that all masses are positive, or revert masses and raise
      # an exception
      for i, mass in enumerate(self.parm.parm_data['MASS']):
         if mass <= 0 and self.parm.atom_list[i].atomic_number > 0:
            ...
            raise HMassRepartitionError('Too much mass removed from atom %d. '
                                        'Hydrogen masses must be smaller.' % i)
...
class openmm(Action):
   ...
   simulation using the OpenMM Python API. It uses a topology file that has been
   defined (and/or modified) here. The command-line flags are identical to those
   used for sander and pmemd (described in the Amber manual). You can
   additionally specify a computational platform available through OpenMM such
   as CUDA, OpenCL, Reference, and CPU.

   The default prmtop will be the 'active' parm. The prmtop can be changed using
   either the 'parm' keyword or the '-p' flag, but it must resolve to one of the
   stored topology files. Any parm given with the '-p' flag has precedence. If
   present, the 'dcd' keyword triggers writting DCD-formatted trajectory files
   instead of NetCDF when ioutfm=1. The DCD trajectory writing offers binary
   trajectory support without requiring a NetCDF-Python library.
   ...
   def execute(self):
      ...
      if not hasattr(self.parm, 'rst7') and not inptraj and not has_inpcrd:
         raise SimulationError('No input coordinates provided for simulation.')
...
class energy(Action):
   ...
   igb <IGB> : An integer corresponding to the desired GB model. May be 1, 2, 5,
         7, or 8 as described by the sander and pmemd manual. Default 5
