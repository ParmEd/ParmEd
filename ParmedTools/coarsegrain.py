...
class Angle(object):
   ...
   def __eq__(self, other):
      """
      Defines if 2 angles are equal: the middle atom has to be the same, and the
      2 outer ones have to be the same in either order, or they can be wildcards
      (X)
      """
      ...
      if not reversed_angles:
         if self.atom3 != other.atom3 and self.atom3 != 'X' and other.atom3 != 'X':
            return False
      else: # reversed angles
         if self.atom3 != other.atom1 and self.atom3 != 'X' and other.atom1 != 'X':
            return False
...
def addCoarseGrain(parm, param_file):
   from ParmedTools.exceptions import CoarseGrainError
   ...
   while file_line:
      ...
      if reading_angles:
         ...
         try:
            ...
            angle_params[idx] = Angle(atom_types[0], atom_types[1],
                                      atom_types[2], acoef, bcoef, ccoef, dcoef)
         except ValueError, err:
            raise CoarseGrainError('Unexpected format in Coarse Grain angles. '
               'Expected different data type: %s. See format specification'
               % err)
         except IndexError:
            raise CoarseGrainError('Unexpected format in Coarse Grain '
               'parameter file. Expected more data fields on the line. '
      ...
      if reading_dihedrals:
         ...
         try:
            ...
            dihedral_params[idx] = Dihedral(atom_types[0], atom_types[1], atom_types[2],
                                            atom_types[3], ampl1, ampl2, ampl3, ampl4,
                                            phase1, phase2, phase3, phase4)
         except ValueError, err:
            raise CoarseGrainError('Unexpected format in Coarse Grain '
               'dihedrals. Expected different data type: %s. See format '
               'specification' % err)
         except IndexError:
            raise CoarseGrainError('Unexpected format in Coarse Grain '
               'parameter file. Expected more data fields on the line. See '
               'format specification')
         file_line = params.readline()
         continue
      print >> stderr, ('Warning: Line (%s) ignored in Coarse Grain parameter '
                        'file ' % file_line)
      file_line = params.readline()
   # End reading while

   # Now let's add comments to our topology file!
   parm.parm_comments['ANGLE_FORCE_CONSTANT'].append('This section is only an '
               'index and is not used for Coarse grained topologies')
   parm.parm_comments['DIHEDRAL_FORCE_CONSTANT'].append('This section is only '
               'an index and is not used for Coarse grained topologies')
   parm.parm_comments['DIHEDRAL_PERIODICITY'].append('This section is not used '
      'for Coarse grained topologies')
   parm.parm_comments['DIHEDRAL_PHASE'].append('This section is not used for '
   ...
   parm.addFlag('DIHEDRAL_AMPLITUDE_1','5E16.8',parm.ptr('nptra'),
      comments='1st Dihedral Amplitude for coarse grained force field')
   parm.addFlag('DIHEDRAL_AMPLITUDE_2','5E16.8',parm.ptr('nptra'),
      comments='2nd Dihedral Amplitude for coarse grained force field')
   parm.addFlag('DIHEDRAL_AMPLITUDE_3','5E16.8',parm.ptr('nptra'),
      comments='3rd Dihedral Amplitude for coarse grained force field')
   parm.addFlag('DIHEDRAL_AMPLITUDE_4','5E16.8',parm.ptr('nptra'),
      comments='4th Dihedral Amplitude for coarse grained force field')
   ...
   for i in range(len(parm.parm_data['ANGLE_FORCE_CONSTANT'])):
      try: 
         ...
         raise CoarseGrainError('Missing angle parameters for index %d' % int(index))
   ...
   for i in range(len(parm.parm_data['DIHEDRAL_FORCE_CONSTANT'])):
      try:
         ...
         raise CoarseGrainError('Missing dihedral parameters for index %d' % int(index))
