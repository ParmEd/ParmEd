...
def lmod(root, amber_prmtop, messages):
   ...
   response = askyesno('lmod',
                 'Do you want to adjust LJ A-coefficients for use with lmod?')
   if response:
...
def hmasschange(root, amber_prmtop, messages):
   """
   Repartitions mass by making H-atoms heavier and attached heteroatoms lighter
   by the same amount to preserve the same total mass
   """
   # The spinbox is sent with the Spinbox, label, and then a list of all of the
   # values to give to it
   ...
   description = ('Repartitions atomic masses to keep total mass the same '
                  'while increasing H-atom masses to allow using a larger time '
                  'step.')
   # Create the window, open it, then wait for it to close
