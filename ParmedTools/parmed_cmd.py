...
class ParmedCmd(cmd.Cmd):
   ...
   def populate_actions(cls):
      ...
      auto-complete. This eliminates the need to modify the ParmedCmd class when
      a new command is added
      """
      for _cmd in ParmedActions.Usages:
         if _cmd in ('source', 'go', 'EOF', 'quit', 'help', 'parmout'): continue
   ...
   def do_source(self, line):
      ...
      _cmd.use_rawinput = 0
      # If we exit on error, call cmdloop without try protection. Otherwise, we
      # need to protect cmdloop to catch any ParmError's that are passed through
      # to avoid exiting the top-level interpreter
      if self._exit_on_fatal:
         _cmd.cmdloop()
      else:
         # If we are not exiting on error, just ignore these parm errors since
         # they've already been printed. Now just wait for the next command from
         # the user.
         try:
            _cmd.cmdloop()
         except ParmError:
         ...
         if char in whitespace:
            ...
            if not word:
               ...
               if nopen > 0:
                  ...
                  i += 1
                  continue
         # This is not whitespace. If it is an open brace, increment nopen. If
         # it is a closing brace, decrement nopen (check that it does not go
         # below zero)
