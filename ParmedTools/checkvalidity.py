...
def check_validity(parm, warnings):
   ...
   # Check relative error, though, since Lennard Jones coefficients are very large
   parm.fill_LJ()
   try:
            ...
            if acoef == 0 or bcoef == 0:
               if acoef != 0 or bcoef != 0 or (wdij != 0 and rij != 0):
                  warnings.warn('Modified off-diagonal LJ parameters detected! '
                        'Will not work with OpenMM.', NonUniversalWarning)
                  raise StopIteration
   ...
   for i, sel in enumerate(mask.Selection()):
      ...
      # We expect 2 bonds
      bondedatms = [a.atname for a in atm.bonds()]
