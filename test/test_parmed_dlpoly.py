...
class TestDlpoly(FileIOTestCase):
    ...
    def test_ala_gas(self):
        ...
        amber.save(output1)
        self.assertTrue(
            diff_files(get_saved_fn('ala_gas.field'), output1,
                       absolute_error=1e-6)
        )
        amber.save(output2)
        self.assertTrue(
            diff_files(get_saved_fn('ala_gas.config'), output2,
                       absolute_error=1e-6)
        )
    ...
    def test_ksi_gas(self):
      ...
      amber.save(output1)
      self.assertTrue(
          diff_files(get_saved_fn('ksi_gas.field'), output1,
                     absolute_error=1e-6)
      )
      amber.save(output2)
      self.assertTrue(
          diff_files(get_saved_fn('ksi_gas.config'), output2,
                     absolute_error=1e-6)
      )
    ...
    def test_ksi_solv(self):
      ...
      amber.save(output1)
      self.assertTrue(
          diff_files(get_saved_fn('ksi_solv.field'), output1,
                     absolute_error=1e-6)
      )
      amber.save(output2)
      self.assertTrue(
          diff_files(get_saved_fn('ksi_solv.config'), output2,
                     absolute_error=1e-6)
      )
