...
class TestGromacsTop(utils.TestCaseRelative):
    ...
    def testTiny(self):
        ...
        # create the system and context, then calculate the energy
        # decomposition
        ...
        energies = energy_decomposition(
            top, context, nrg=u.kilojoules_per_mole)
    ...
    def testSmallPeptide(self):
        ...
        gmxfrc = get_forces_from_xvg(
            os.path.join(get_fn('04.Ala'), 'force.xvg'))
        ommfrc = context.getState(getForces=True).getForces().value_in_unit(
            u.kilojoules_per_mole / u.nanometer)
        zero_ep_frc(ommfrc, top)
        max_diff = get_max_diff(gmxfrc, ommfrc)
    ...
    def testJAC(self):
        ...
        # Load the top and gro files
        top = load_file(
            os.path.join(get_fn('07.DHFR-Liquid-NoPBC'), 'topol.top'))
        gro = load_file(
        ...
        self.assertRelativeEqual(
            energies['nonbonded'], -288718.981405, places=4)
        gmxfrc = get_forces_from_xvg(
    ...
    def testJACPME(self):
        ...
        self.assertRelativeEqual(
            energies['nonbonded'], 23616.457584, delta=0.002)
        gmxfrc = get_forces_from_xvg(
    ...
    def testJACPMESwitch(self):
        ...
        # Load the top and gro files
        top = load_file(
            os.path.join(get_fn('10.DHFR-PME-Switch'), 'topol.top'))
    ...
    def testDPPC(self):
        ...
        self.assertRelativeEqual(
            energies['nonbonded'], -16432.8092955, places=4)
        gmxfrc = get_forces_from_xvg(
